...
export default async function CartPage() {
    ...
    return (
        ...
        <div className="flex flex-row">
            ...
            {cartItems.map((item) => <ProductCard key={item.id} path="/kosik" productId={item.product_id} variant="cartShowcase"/>)}
            </div>
            <div className="flex flex-row gap-2 items-center">
                ...
                <p className="text-muted-foreground text-[0.6rem] pt-1">vrátane DPH</p>
            </div>
            ...
            <Button variant='default' className="w-full py-2">Zaplatiť a objednať</Button>
            <Button variant='outline' className="w-full py-2 -mt-1">Pokračovať v nakupovaní</Button>
            </div>
        </div>
        </>
    )
